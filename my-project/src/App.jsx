// src/App.jsx - Add this to your existing App.jsx
...
import AdminAppointments from './components/AdminAppointments.jsx'; // NEW

// Import Lucide React Icons
import { Home, PawPrint, CalendarDays, Settings, LogOut, Menu, X, Sun, Moon, AlertTriangle, ChevronLeft, UserCircle, Shield } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-gray-900 to-pink-900 flex items-center justify-center p-6">
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-8 text-center border border-red-500/30 max-w-md">
          ...
          <p className="text-purple-300 mb-6">You need admin privileges to access this page.</p>
          <button 
...
function Navigation() {
  ...
  if (user?.role === 'admin') {
    navigation.push({ path: '/admin/appointments', name: 'Admin Panel', icon: Shield });
  }
    ...
    return (
      <button
        ...
        className={`
          ...
          ${isMobile 
              ? `block rounded-md mt-1 ${isActive(item.path) ? 'bg-blue-700 text-white' : 'text-gray-300 hover:bg-gray-700'}`
              : `${isActive(item.path) 
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'} transition-colors duration-300`}>
      <Routes>
        ...
        <Route 
          path="/login" 
          element={user ? <Navigate to="/" replace /> : <Login onLoginSuccess={login} />} 
        />
        <Route 
          path="/register" 
          element={user ? <Navigate to="/" replace /> : <Register onRegisterSuccess={login} />} 
        />
